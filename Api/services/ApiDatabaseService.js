...
var ApiDatabaseService = {
    ...
    SearchRecipeByIngredient: async function(IngredientName){
        const con = await this.getConnection();
        rows = await con.execute('Select * from RECIPE a inner join RECIPETOINGREDIENT b on (a.ID = b.RECIPE_ID) inner join INGREDIENT c on (b.INGREDIENT_ID = c.ID) where c.NAME = ?',[IngredientName]);
        return rows;
    }
